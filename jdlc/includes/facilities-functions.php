...
<a href="'.$a['link'].'">
<div class="facility">
  <div class="header">'. strtoupper($a['name']);
  if($a['longname'] != ''){ $returnString .= '<span class="longname">('.$a['longname'].')</span>'; }
  return $returnString . '</div>
  <span class="human">'.$a['manager'].'</span>
...
function header_shortcode($atts, $content) {
  return '<h1 class="header">'. strtoupper($content) .'</h1>';
}
...
function grid_item_shortcode($atts, $content) {
  ...
  return '<div class="grid_item">
    <div class="header">'. strtoupper($a['header']) .'</div>
